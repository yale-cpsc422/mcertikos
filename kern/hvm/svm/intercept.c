...
void set_iopm_intercept(uint64_t * iopmtable, uint16_t ioport, bool enable);
...
//	cprintf("n_cr3 is : %x, va is :  %x\n", vm->vmcb->n_cr3, va);// PGADDR((unsigned long)vm->vmcb->exitinfo2));
	as_reserve((as_t *)(uint32_t)vm->vmcb->n_cr3,(uint32_t) vm->vmcb->exitinfo2, PTE_W|PTE_U|PTE_G);
}
