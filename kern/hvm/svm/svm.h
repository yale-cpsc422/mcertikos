...
/*
 * Types of intercept operations.
 *
 * XXX: DO NOT ALTER THE ORDER OF ITEMS.
 *      The order of items are consistent to the order they appear in the
 *      intercept field in VMCB.
 */
enum {
	INTERCEPT_INTR,
	INTERCEPT_NMI,
...
/*
 * Segment structure for segment registers in VMCB.
 */
struct vmcb_seg {
	uint16_t 	selector;
	uint16_t 	attrib;
...
/*
 * The machine-dependent state of VM.
 *
 * XXX: DO NOT ALTER THE ORDER OF THE ITEMS IN THE FIRST TWO LINES.
 *      svm_run() relies on this geography to save and restore the VM state
 *      before entering and after exiting VM.
 */
struct svm {
	/*
