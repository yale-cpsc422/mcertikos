...
//#define time(a) (unsigned)sys_time()
...
#define TID 0
#define HEAP_SZ VMMSTACK_SIZE
#define HEAP_LO VMMSTACK_LO
static mspace child_mspace[MAXTHREADS];
static inline void *
mspace_alloc_pages()
{
	//void *mem = (void*)(HEAP_LO + HEAP_SZ * TID);
	void *mem = (void*)(HEAP_LO);
...
thmalloc(size_t bytes)
{
	...
	return mspace_malloc(child_mspace[TID], bytes);
}
...
thfree(void *mem)
{
	...
	mspace_free(child_mspace[TID], mem);
}

void *
thcalloc(size_t num, size_t bytes)
{
	void *ret = thmalloc(num * bytes);
...
#else
        magic = (size_t)(timer_read() ^ (size_t)0x55555555U);
#endif
...
static void internal_malloc_stats(mstate m) {
  ...
  if (!PREACTION(m)) {
    ...
    cprintf( "max system bytes = %10lu\n", (unsigned long)(maxfp));
    cprintf( "system bytes     = %10lu\n", (unsigned long)(fp));
    cprintf( "in use bytes     = %10lu\n", (unsigned long)(used));
...
void* mspace_malloc(mspace msp, size_t bytes) {
  ...
  if (!PREACTION(ms)) {
    ...
    //mem = sys_alloc(ms, nb);
	cprintf("heap overflow for thread %d\n", TID);
