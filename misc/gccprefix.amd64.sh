...
if pios-objdump -i 2>&1 | grep '^elf64-x86-64$' >/dev/null 2>&1; then
	echo 'pios-'
elif i386-elf-objdump -i 2>&1 | grep '^elf64-x86-64$' >/dev/null 2>&1; then
	echo 'i386-elf-'
elif objdump -i 2>&1 | grep 'elf64-x86-64' >/dev/null 2>&1; then
	echo ''
else
	echo "***" 1>&2
