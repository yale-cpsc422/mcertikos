		   ...
		   uint32_t offset)
{
	...
	KERN_ASSERT(dev < 32);
	KERN_ASSERT(func < 8);
	KERN_ASSERT(offset < 256);
	KERN_ASSERT((offset & 0x3) == 0);
	...
	for (i = 0; list[i].attachfn; i++) {
		if (list[i].key1 == key1 && list[i].key2 == key2) {
			...
			if (r > 0)
				return r;
			if (r < 0)
				KERN_INFO("pci_attach_match: attaching "
					  "%x.%x (%p): e\n",
...
static void
pci_init_device(struct pci_func *f)
{
	KERN_ASSERT(f != NULL);

	uint32_t class = PCI_CLASS(f->dev_class);
	uint32_t subclass = PCI_SUBCLASS(f->dev_class);
...
pci_scan_bus(struct pci_bus *bus)
{
	...
	for (df.dev = 0; df.dev < 32; df.dev++) {
		...
		for (f.func = 0; f.func < (PCI_HDRTYPE_MULTIFN(bhlc) ? 8 : 1);
			...
			if (pci_show_devs &&
			    PCI_CLASS(af.dev_class) == PCI_CLASS_MASS_STORAGE)
				pci_print_func(&af);
			pci_attach(&af);
...
pci_bridge_attach(struct pci_func *pcif)
{
	...
	if (PCI_BRIDGE_IO_32BITS(ioreg)) {
		KERN_INFO("PCI: %02x:%02x.%d: 32-bit bridge IO not supported.\n",
	...
	if (pci_show_devs)
		KERN_INFO("PCI: %02x:%02x.%d: bridge to PCI bus %d--%d\n",
			  pcif->bus->busno, pcif->dev, pcif->func,
			  nbus.busno,
