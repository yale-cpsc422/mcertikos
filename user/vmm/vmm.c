...
vmm_init_vm (struct vm *vm, uint64_t cpufreq, size_t memsize)
{
	...
	if (cpufreq >= tsc_per_ms * 1000ull)
	{
		DEBUG("Guest CPU frequency cannot be higher than"
			" the host CPU frequency. (guest=%llu, host=%llu)\n", cpufreq,
			tsc_per_ms * 1000);
		return -1;
	}
...
vmm_run_vm (struct vm *vm)
{
	...
	while (1)
	{
		...
		sys_hvm_get_exitinfo (vm->vmid, &vm->exit_reason, &vm->exit_info);
		...
		{
			VMM_DEBUG("Cannot handle a VM exit (exit_reason %d).\n",
				vm->exit_reason);
			rc = -3;
			break;
		}
	...
	if (rc)
		VMM_DEBUG("Virtual machine terminates abnormally.\n");
	return rc;
}
						...
						int write)
{
	...
	{
		VMM_DEBUG("Size (%d bytes) is out of range (%d bytes).\n", size,
			vm->memsize - gpa);
		return 1;
	}
